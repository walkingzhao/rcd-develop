...
typedef struct
{
    uint8_t front_service_brake_percent; //前制动百分比反馈信号

    uint8_t auto_nomous_mode : 2; //无人/人工驾驶模式信号
    uint8_t emergency_brake : 2; //紧急制动信号
    uint8_t load_brake : 2; //装载制动-实施/解除反馈信号
    uint8_t park_brake : 2; //驻车制动-实施/解除反馈信号

    uint8_t ShiftNDR : 2; //档位反馈量
    uint8_t res1 : 6;

    uint16_t res2;

    uint8_t steering_valve_current_operation_state; //转向阀当前状态反馈
    uint8_t res3;
    uint8_t res4;
} NT0x18FE0821Struct;
...
typedef struct
{
    uint8_t throttle_pedal_apply_feedback; //上位机油门执行反馈
    uint8_t throttle_pedal_feedback; //油门踏板反馈
    uint8_t rear_service_brake_percent; //后制动百分比反馈信号
    uint16_t steering_angle; //车辆转角反馈
    uint8_t retard_Pedal_apply_feedback; //上位机缓行执行反馈
    uint8_t retard_pedal_feedback; //缓行踏板反馈
    uint8_t res1;
} NT0x18FE0921Struct;

typedef struct
{
    uint16_t front_left_wheel_speed; //左前轮转速
    uint16_t front_right_wheel_speed; //右前轮转速
    uint16_t rear_left_wheel_speed; //左后轮转速
    uint16_t rear_right_wheel_speed; //右后轮转速
} NT0x18FE0B21Struct;

typedef struct
{
    uint16_t truck_speed; //车速
    uint16_t engine_speed; //发动机转速
    uint16_t battery_voltage; //电池电压
    uint8_t engine_coolant_temperature; //发动机冷却液温度
    uint8_t engine_oil_pressure; //发动机机油压力
} NT0x18FE0C21Struct;
...
union CanFrameMsg
{
    ...
    NT0x18FE0821Struct nt0x18fe0821St;
    NT0x18FE0921Struct nt0x18fe0921St;
    NT0x18FE0B21Struct nt0x18fe0b21St;
    NT0x18FE0C21Struct nt0x18fe0c21St;
