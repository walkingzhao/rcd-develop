    ...
    ui(new Ui::RightWgt)
{
    ...
    m_pCurrentWgt[SA_RightScreenL] = pWgt; //右屏幕左区域
    m_pHLayout->addStretch(2);

    pWgt = WgtFactory::getFunWidget(MD_WarnInfo);
    m_pHLayout->addWidget(pWgt,40);
    m_pCurrentWgt[SA_RightScreenR] = pWgt; //右屏幕右区域

//    QWidget *pWgt = WgtFactory::getFunWidget(MD_MapWeb);
//    m_pHLayout->addWidget(pWgt);
//    m_pCurrentWgt[SA_RightScreen] = pWgt;
    m_pHLayout->addStretch(2);
