    ...
    ui(new Ui::LeftCameraWgt)
{
    ...
    QString str(QString::fromUtf8("【xxxx】 左后摄像头"));
...
//    qDebug()<<"左后摄像头url:"<<m_pModel->m_url;
...
void LeftCameraWgt::updateWidget()
{
   qInfo()<<"updatexzk LeftCameraWgt::updateWidget";
    ui->playWgt->startPlay(m_pModel->m_url);
    m_pTitleWgt->setTitleString(m_pModel->m_name);
    this->update();
}
...
void LeftCameraWgt::onClDataCome(int cmd, QVariant data)
{
    data.isValid();
    switch (cmd) {
    case LeftCameraCL::CL_StartPlay:
    {
        if(data.toBool())
        {
            qInfo()<<"updatexzk LeftCameraWgt::startPlay";
            m_pCL->intoAction();
            this->updateWidget();
        }else
        {
            qInfo()<<"updatexzk LeftCameraWgt::stopPlay";
            ui->playWgt->stopPlay();
