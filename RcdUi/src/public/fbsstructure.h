...
struct vehicleBodysST{
    int8_t vehicleType;             //车辆类型 1:电车 2:油车 3:冲尘车
    ...
    float coolantLevel;           //冷却液液位 
    float exhaustPortEmperature; //排气口温度
    float temperature;           //温度
    float waterTankLevel;        //水箱水位
...
//    uint8_t handBrake;            //手制动电磁阀 0x01: open 0x00: close
    ...
    friend QDataStream &operator<<(QDataStream &out, const vehicleBodysST &st)
    {
        QByteArray tmp((const char*)&st,sizeof(st));
        out<<tmp;
        return out;
    }
    friend QDataStream &operator>>(QDataStream &in, vehicleBodysST &st)
    {
        QByteArray tmp;
        in>>tmp;
...
Q_DECLARE_METATYPE(vehicleBodysST)

struct vehicleLightsST{
    ...
    friend QDataStream &operator<<(QDataStream &out, const vehicleLightsST &st){
        QByteArray tmp((const char*)&st,sizeof(st));
        out<<tmp;
        return out;
    }
    friend QDataStream &operator>>(QDataStream &in, vehicleLightsST &st){
        QByteArray tmp;
        in>>tmp;
        memcpy(&st,tmp.data(),tmp.size());
        return in;
...
Q_DECLARE_METATYPE(vehicleLightsST)

//车端上报车辆反馈信息
struct vehicleDataST {
    vehicleBodysST bodys;
    vehicleLightsST lights;
...
//self test CtrlRawData struct
struct SelfTest_CtrlRawData_St {
    uint16_t clutch;         //离合
    uint16_t brake;          //刹车
    ...
    friend QDataStream &operator<<(QDataStream &out, const SelfTest_CtrlRawData_St &st)
    {
        QByteArray tmp((const char*)&st,sizeof(st));
        out<<tmp;
        return out;
    }
    friend QDataStream &operator>>(QDataStream &in, SelfTest_CtrlRawData_St &st)
    {
        QByteArray tmp;
        in>>tmp;
        memcpy(&st,tmp.data(),tmp.size());
...
Q_DECLARE_METATYPE(SelfTest_CtrlRawData_St)
...
struct VehicleCtrlCmd{
    ...
    float brakePedal;           //电制动踏板位置百分比 0~100
    float hydraulicBrake;       //机械制动踏板反馈百分比 0~100
    uint8_t retarderCtrl;       //缓速器控制
    uint8_t exhaustBrake;       //排气制动0x01: enable 0x00: disable
    uint8_t parkCtrl;           //停车制动控制.0:解除驻车;1:驻车  手刹
    uint8_t bucketCtrl;         //货箱控制.0:保持 1:上升 2:下降
    uint8_t shiftCtrl;          //档位控制.2: D; 3: N; 4: R
    bool eStop;                 //急停控制.true:急停按下;false:急停复位
    uint8_t loadingCtrl;        //装载制动控制.0:解除驻车;1:驻车 P档

    friend QDataStream &operator<<(QDataStream &out, const VehicleCtrlCmd &st)
    {
        QByteArray tmp((const char*)&st,sizeof(st));
        out<<tmp;
        return out;
    }
    friend QDataStream &operator>>(QDataStream &in, VehicleCtrlCmd &st)
    {
        QByteArray tmp;
        in>>tmp;
        memcpy(&st,tmp.data(),tmp.size());
...
Q_DECLARE_METATYPE(VehicleCtrlCmd)
...
struct LightCtrlCmd{
    ...
    uint8_t brake;          //制动灯
    uint8_t back;           //倒车灯
    uint8_t rearStatus;     //后工作灯

    friend QDataStream &operator<<(QDataStream &out, const LightCtrlCmd &st){
        QByteArray tmp((const char*)&st,sizeof(st));
        out<<tmp;
        return out;
    }
    friend QDataStream &operator>>(QDataStream &in, LightCtrlCmd &st){
        QByteArray tmp;
        in>>tmp;
        memcpy(&st,tmp.data(),tmp.size());
        return in;
...
Q_DECLARE_METATYPE(LightCtrlCmd)

//车端上报车辆反馈信息
struct SelfTest_ControlData_St {
    VehicleCtrlCmd vehicleCmd;
    LightCtrlCmd lightCmd;

    friend QDataStream &operator<<(QDataStream &out, const SelfTest_ControlData_St &st)
    {
        QByteArray tmp((const char*)&st,sizeof(st));
        out<<tmp;
        return out;
    }
    friend QDataStream &operator>>(QDataStream &in, SelfTest_ControlData_St &st)
    {
        QByteArray tmp;
        in>>tmp;
        memcpy(&st,tmp.data(),tmp.size());
...
Q_DECLARE_METATYPE(SelfTest_ControlData_St)
